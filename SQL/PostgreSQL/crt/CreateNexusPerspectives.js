...
-- @reliable            whether to show reliable (1) or unreliable (0) rows
--
...
ON
    [$attribute.mnemonic].$attribute.identityColumnName = (
        ...
        AND
            sub.$attribute.reliableColumnName = isnull(@reliable, sub.$attribute.reliableColumnName)
        ORDER BY
...
CROSS APPLY
    [$nexus.capsule].[t$nexus.name](p.$schema.metadata.positorSuffix, timepoints.inspectedTimepoint, DEFAULT, 1) [$nexus.mnemonic]
WHERE
    @selection is null
OR
    EXISTS (
