...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:param name="namingConvention">
        ...
        <xsl:for-each select="anchor">
            ...
            <xsl:for-each select="attribute">
                ...
                <xsl:variable name="knotId">
                    <xsl:choose>
                        <xsl:when test="$namingConvention = 'improved'">
                ...
                <xsl:for-each select="attribute">
                    ...
                    <xsl:variable name="knotOrDataDefinition">
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:value-of select="concat($T, $T, $attributeName, ' ', @dataRange, ' null')"/>
                            </xsl:otherwise>
                        </xsl:choose>
                        <xsl:if test="not(position() = last())">
                    ...
                    <xsl:variable name="attributeChangingDefinition">
                        <xsl:if test="@timeRange">
                            <xsl:value-of select="concat($T, $T, $attributeMnemonic, '_', $changingSuffix, ' ', @timeRange, ' null,', $N)"/>
                        </xsl:if>
                    </xsl:variable>
                    <xsl:variable name="attributeRecordingDefinition">
                        <xsl:if test="$temporalization = 'bi'">
                            <xsl:value-of select="concat(
                            $T, $T, $attributeMnemonic, '_', $recordingSuffix, ' ', $recordingRange, ' null,', $N,
                            $T, $T, $attributeMnemonic, '_', $erasingSuffix, ' ', $recordingRange, ' null,', $N
                            )"/>
                        </xsl:if>
                    </xsl:variable>
                    <xsl:value-of select="concat(
            ...
            <xsl:variable name="columnInserts">
                <xsl:for-each select="attribute">
                    <xsl:variable name="attributeMnemonic" select="concat($anchorMnemonic, '_', @mnemonic)"/>
                    <xsl:variable name="attributeMetadata" select="concat($metadataPrefix, '_', $attributeMnemonic)"/>
                    <xsl:variable name="attributeName" select="concat($attributeMnemonic, '_', parent::*/@descriptor, '_', @descriptor)"/>
                    ...
                    <xsl:variable name="knotOrDataInsert">
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:value-of select="concat($T, $T, 'i.', $attributeName)"/>
                            </xsl:otherwise>
                        </xsl:choose>
                        <xsl:if test="not(position() = last())">
                            <xsl:value-of select="concat(',', $N)"/>
                    ...
                    <xsl:variable name="attributeChangingInsert">
                        <xsl:if test="@timeRange">
                            <xsl:value-of select="concat($T, $T, 'ISNULL(i.', $attributeMnemonic, '_', $changingSuffix, ', @now),', $N)"/>
                        </xsl:if>
                    </xsl:variable>
                    <xsl:variable name="attributeRecordingInsert">
                        <xsl:if test="$temporalization = 'bi'">
                            <xsl:value-of select="concat(
                            $T, $T, 'ISNULL(i.', $attributeMnemonic, '_', $recordingSuffix, ', @now),', $N,
                            $T, $T, 'ISNULL(i.', $attributeMnemonic, '_', $erasingSuffix, ', ', $infinity, '),', $N
                            )"/>
                        </xsl:if>
                    </xsl:variable>
                    <xsl:value-of select="concat(
                    $attributeChangingInsert,
                    $attributeRecordingInsert,
                    $attributeMetadataInsert,
                    $knotOrDataInsert
                    )"/>
                </xsl:for-each>
            </xsl:variable>
            <xsl:variable name="columnSelects">
                <xsl:for-each select="attribute">
                    <xsl:variable name="attributeMnemonic" select="concat($anchorMnemonic, '_', @mnemonic)"/>
                    <xsl:variable name="attributeMetadata" select="concat($metadataPrefix, '_', $attributeMnemonic)"/>
                    <xsl:variable name="attributeName" select="concat($attributeMnemonic, '_', parent::*/@descriptor, '_', @descriptor)"/>
                    <xsl:variable name="attributeMetadataSelect">
                        <xsl:if test="$metadataUsage = 'true'">
                    ...
                    <xsl:variable name="knotOrDataSelect">
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:value-of select="concat($T, $T, $T, $attributeName, ',', $N)"/>
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:variable>
                    <xsl:variable name="attributeChangingSelect">
                        <xsl:if test="@timeRange">
                            <xsl:value-of select="concat($T, $T, $T, $attributeMnemonic, '_', $changingSuffix, ',', $N)"/>
                        </xsl:if>
                    </xsl:variable>
                    <xsl:variable name="attributeRecordingSelect">
                        <xsl:if test="$temporalization = 'bi'">
                            <xsl:value-of select="concat(
                            $T, $T, $T, $attributeMnemonic, '_', $recordingSuffix, ',', $N,
                            $T, $T, $T, $attributeMnemonic, '_', $erasingSuffix, ',', $N
                    ...
                    <xsl:value-of select="concat(
                    $attributeChangingSelect,
                    $attributeRecordingSelect,
                    $attributeMetadataSelect,
                    $knotOrDataSelect
                    )"/>
                </xsl:for-each>
            </xsl:variable>
            <xsl:variable name="insertedTable">
                <xsl:choose>
                    <xsl:when test="string(metadata/@generator) = 'true'">
                        <xsl:variable name="metadataSelect">
                            <xsl:if test="$metadataUsage = 'true'">
                                <xsl:value-of select="concat($T, $T, $T, $anchorMetadata, ',', $N)"/>
                            </xsl:if>
                        </xsl:variable>
                        <xsl:variable name="metadataDefinition">
                            <xsl:if test="$metadataUsage = 'true'">
                                <xsl:value-of select="concat($T, $T, $anchorMetadata, ' ', $metadataType, ' not null,', $N)"/>
                            </xsl:if>
                        </xsl:variable>
                        <xsl:variable name="metadataInsert">
                            <xsl:if test="$metadataUsage = 'true'">
                                <xsl:value-of select="concat($T, $T, 'i.', $anchorMetadata, ', ', $N)"/>
                            </xsl:if>
                        </xsl:variable>
                        <xsl:value-of select="concat(
                    ...
                    <xsl:otherwise>
                        ...
                        $T, $T, 'inserted i', $N,
                        $T, 'WHERE', $N,
                        $T, $T, 'i.', $anchorIdentity, ' is not null;', $N
                        )"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:variable>
    ...
    <xsl:template name="joinCondition">
        ...
        <xsl:value-of select="concat($N, 'ON', $N, $T, '[',  $attributeMnemonic, '].', $attributeIdentity, ' = [', $anchorMnemonic, '].', $anchorIdentity)"/>
        <xsl:if test="$attribute/@timeRange">
            ...
            <xsl:value-of select="concat($N, $T, $T, 'WHERE', $N, $T, $T, $T, 'sub.', $attributeIdentity, ' = [', $anchorMnemonic, '].', $anchorIdentity)"/>
            <xsl:value-of select="concat($N, $T, ')')"/>
        </xsl:if>
        <xsl:if test="key('knotLookup', $attribute/@knotRange)">
            ...
            <xsl:variable name="attributeKnotId">
                <xsl:choose>
                    <xsl:when test="$namingConvention = 'improved'">
                        <xsl:value-of select="concat($attributeMnemonic, '_', $knotIdentity)"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="$knotIdentity"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:variable>
            <xsl:value-of select="concat($N, 'LEFT JOIN', $N, $T, $knotSource, ' [',  $attributeMnemonic, '_', $knotMnemonic, ']')"/>
            <xsl:value-of select="concat($N, 'ON', $N, $T, '[',  $attributeMnemonic, '_', $knotMnemonic, '].', $knotIdentity, ' = [', $attributeMnemonic, '].', $attributeKnotId)"/>
        </xsl:if>
    </xsl:template>
    <xsl:template name="columnReference">
        ...
        <xsl:choose>
            <xsl:when test="key('knotLookup', @knotRange)">
                ...
                <xsl:variable name="attributeKnotName">
                    <xsl:choose>
                        <xsl:when test="$namingConvention = 'improved'">
                            <xsl:value-of select="concat($attributeMnemonic, '_', $knotName)"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="$knotName"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>
                <xsl:variable name="attributeKnotMetadata">
                    <xsl:choose>
                        <xsl:when test="$namingConvention = 'improved'">
                            <xsl:value-of select="concat($attributeMnemonic, '_', $knotMetadata)"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="$knotMetadata"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>
                <xsl:value-of select="concat(',', $N, $T, '[', $attributeMnemonic, '_', $knotMnemonic, '].', $knotIdentity, ' AS ', $attributeKnotId)"/>
                <xsl:value-of select="concat(',', $N, $T, '[', $attributeMnemonic, '_', $knotMnemonic, '].', $knotName, ' AS ', $attributeKnotName)"/>
                <xsl:if test="$metadataUsage = 'true'">
                    <xsl:value-of select="concat(',', $N, $T, '[', $attributeMnemonic, '_', $knotMnemonic, '].', $knotMetadata, ' AS ', $attributeKnotMetadata)"/>
                </xsl:if>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template name="insertStatement">
        ...
        <xsl:variable name="anchorIdentity" select="concat($anchorMnemonic, '_', $identitySuffix)"/>
        <xsl:variable name="anchorIdentityType" select="$anchor/@identity"/>
        <xsl:variable name="attributeMnemonic" select="concat($anchorMnemonic, '_', $attribute/@mnemonic)"/>
        <xsl:variable name="attributeMetadata" select="concat($metadataPrefix, '_', $attributeMnemonic)"/>
        <xsl:variable name="attributeName" select="concat($attributeMnemonic, '_', $anchor/@descriptor, '_', $attribute/@descriptor)"/>
        <xsl:variable name="attributeCapsule" select="$attribute/metadata/@capsule"/>
        ...
        <xsl:variable name="attributeHistorizationDefinition">
            <xsl:if test="$attribute/@timeRange">
                <xsl:value-of select="concat(', ', $N, $T, $T, $attributeMnemonic, '_', $changingSuffix, ' ', $attribute/@timeRange, ' not null')"/>
            </xsl:if>
        </xsl:variable>
        <xsl:variable name="attributeHistorizationInsert">
            <xsl:if test="$attribute/@timeRange">
                <xsl:value-of select="concat(', ', $N, $T, $T, 'i.', $attributeMnemonic, '_', $changingSuffix)"/>
            </xsl:if>
        </xsl:variable>
        <xsl:variable name="attributeMetadataColumn">
            <xsl:if test="$metadataUsage = 'true'">
        ...
        <xsl:variable name="attributeMetadataDefinition">
            <xsl:if test="$metadataUsage = 'true'">
                <xsl:value-of select="concat(', ', $N, $T, $T, $attributeMetadata, ' ', $metadataType, ' not null')"/>
            </xsl:if>
        </xsl:variable>
        <xsl:variable name="attributeMetadataInsert">
            <xsl:if test="$metadataUsage = 'true'">
                <xsl:value-of select="concat(', ', $N, $T, $T, 'i.', $attributeMetadata)"/>
            </xsl:if>
        </xsl:variable>
        ...
        <xsl:variable name="attributeRecordingDefinitions">
            <xsl:if test="$temporalization = 'bi'">
                <xsl:value-of select="concat(
                ', ', $N, $T, $T, $attributeMnemonic, '_', $recordingSuffix, ' ', $recordingRange, ' not null,', $N,
                $T, $T, $attributeMnemonic, '_', $erasingSuffix, ' ', $recordingRange, ' not null'
                )"/>
            </xsl:if>
        </xsl:variable>
        <xsl:variable name="attributeRecordingInserts">
            <xsl:if test="$temporalization = 'bi'">
                <xsl:value-of select="concat(
                ',', $N, $T, $T, 'i.', $attributeMnemonic, '_', $recordingSuffix, ',', $N,
                $T, $T, 'i.', $attributeMnemonic, '_', $erasingSuffix
                )"/>
            </xsl:if>
        </xsl:variable>
        <xsl:variable name="namingPrefix">
            <xsl:if test="$namingConvention = 'improved'">
                <xsl:value-of select="concat($attributeMnemonic, '_')"/>
            </xsl:if>
        </xsl:variable>
        <xsl:choose>
            <xsl:when test="$attribute/@knotRange">
                ...
                <xsl:variable name="knotIdentity" select="concat($knotMnemonic, '_', $identitySuffix)"/>
                <xsl:choose>
                    <xsl:when test="$attribute/@timeRange and $attribute/metadata/@idempotent = 'true'">
                        <xsl:value-of select="concat(
                        $T, 'DECLARE @', $attributeName, ' TABLE (', $N,
                        $T, $T, $namingPrefix, $anchorIdentity, ' ', $anchorIdentityType, ' not null,', $N,
                        $T, $T, $namingPrefix, $knotName, ' ', $knot/@dataRange, ' not null,', $N,
                        $T, $T, $namingPrefix, $knotIdentity, ' ', $knot/@identity, ' not null,', $N,
                        $T, $T, $attributeMnemonic, '_Version int not null',
                        $attributeMetadataDefinition,
                        $attributeHistorizationDefinition,
                        $attributeRecordingDefinitions, $N,
                        $T, $T, 'primary key(', $attributeMnemonic, '_Version, ', $namingPrefix, $anchorIdentity, ')', $N,
                        $T, ')', $N,
                        $T, 'INSERT INTO @', $attributeName, $N,
                        $T, 'SELECT', $N,
                        $T, $T, 'i.', $anchorIdentity, ',', $N,
                        $T, $T, 'i.', $namingPrefix, $knotName, ',', $N,
                        $T, $T, 'ISNULL(i.', $namingPrefix, $knotIdentity, ', [', $knotMnemonic, '].', $knotIdentity, '),', $N,
                        ...
                        $T, 'ON', $N,
                        $T, $T, '[', $knotMnemonic, '].', $knotName, ' = i.', $namingPrefix, $knotName, ';', $N,
                        $T, 'SELECT @maxV = MAX(', $attributeMnemonic, '_Version) FROM @', $attributeName, ';', $N,
                        $T, 'SET @v = 0;', $N,
                        $T, 'WHILE(@v &lt; @maxV)', $N,
                        $T, 'BEGIN', $N,
                        $T, 'SET @v = @v + 1;', $N,
                        $T, 'INSERT INTO [', $attributeCapsule, '].[', $attributeName, '](', $N,
                        $T, $T, $namingPrefix, $anchorIdentity, ', ', $N,
                        $T, $T, $namingPrefix, $knotIdentity,
                        $attributeMetadataColumn,
                        $attributeHistorizationColumn,
                        $attributeRecordingColumns, $N,
                        $T, ')', $N,
                        $T, 'SELECT', $N,
                        $T, $T, 'i.', $namingPrefix, $anchorIdentity, ',', $N,
                        $T, $T, 'i.', $namingPrefix, $knotIdentity,
                        $attributeMetadataInsert,
                        $attributeHistorizationInsert,
                        $attributeRecordingInserts, $N,
                        $T, 'FROM', $N,
                        $T, $T, '@', $attributeName, ' i', $N,
                        $T, 'LEFT JOIN', $N,
                        $T, $T, '[', $anchorCapsule, '].[l', $anchorName, '] [', $anchorMnemonic, ']', $N,
                        $T, 'ON', $N,
                        $T, $T, '[', $anchorMnemonic, '].', $anchorIdentity , ' = i.', $namingPrefix, $anchorIdentity, $N,
                        $T, 'WHERE', $N,
                        $T, $T, 'i.', $attributeMnemonic, '_Version = @v', $N,
                        $T, 'AND (', $N,
                        $T, $T, '[', $anchorMnemonic, '].', $namingPrefix, $knotIdentity, ' is null', $N,
                        $T, 'OR', $N,
                        $T, $T, '[', $anchorMnemonic, '].', $namingPrefix, $knotIdentity, ' &lt;&gt; i.', $namingPrefix, $knotIdentity, $N,
                        $T, ')', $N,
                        $T, 'END', $N
                        )"/>
                    </xsl:when>
                    <xsl:otherwise>
                        ...
                        $T, 'ON', $N,
                        $T, $T, 'k.', $knotName, ' = i.', $namingPrefix, $knotName, $N,
                        $T, 'WHERE', $N,
                        $T, $T, 'i.', $namingPrefix, $knotName, ' is not null;', $N
                        )"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:when>
            <xsl:otherwise>
                <xsl:variable name="attributeDataType" select="$attribute/@dataRange"/>
                <xsl:choose>
                    <xsl:when test="$attribute/@timeRange and $attribute/metadata/@idempotent = 'true'">
                        <xsl:value-of select="concat(
                        $T, 'DECLARE @', $attributeName, ' TABLE (', $N,
                        ...
                        $T, $T, 'i.', $attributeMnemonic, '_Version = @v', $N,
                        $T, 'AND (', $N,
                        $T, $T, '[', $anchorMnemonic, '].', $attributeName, ' is null', $N,
                        $T, 'OR', $N,
                        $T, $T, '[', $anchorMnemonic, '].', $attributeName, ' &lt;&gt; i.', $attributeName, $N,
                        $T, ')', $N,
                        $T, 'END', $N
                        )"/>
                    </xsl:when>
    ...
    <xsl:template name="updateStatement">
        ...
        <xsl:variable name="updatePrevious">
            <xsl:if test="$temporalization = 'bi'">
                ...
                $T, $T, 'inserted i', $N,
                $T, 'ON', $N,
                $T, $T, 'i.', $anchorIdentity, ' = ', $attributeMnemonic, '.', $namingPrefix, $anchorIdentity, $N,
                $attributeChangingCondition,
                $T, 'AND', $N,
        ...
        <xsl:choose>
            <xsl:when test="$attribute/@knotRange">
                ...
                <xsl:variable name="knotIdentity" select="concat($knotMnemonic, '_', $identitySuffix)"/>
                <xsl:variable name="knotCapsule" select="$knot/metadata/@capsule"/>
                <xsl:variable name="idempotencyStatement">
                    <xsl:if test="$attribute/@timeRange and $attribute/metadata/@idempotent = 'true'">
                        <xsl:value-of select="concat(
                        $N, $T, 'LEFT JOIN', $N,
                        $T, $T, 'deleted d', $N,
                        $T, 'ON', $N,
                ...
                $recordingCheck,
                $updatePrevious,
                $T, 'IF(UPDATE(', $namingPrefix, $knotName, ') OR UPDATE(', $namingPrefix, $knotIdentity, '))', $N,
                $T, 'INSERT INTO [', $attributeCapsule, '].[', $attributeName, '](', $N,
                $T, $T, $namingPrefix, $anchorIdentity, ', ', $N,
                $T, $T, $namingPrefix, $knotIdentity,
                $attributeMetadataColumn,
                ...
                $T, $T, '[', $knotCapsule, '].[', $knotName, '] k', $N,
                $T, 'ON', $N,
                $T, $T, 'k.', $knotName, ' = i.', $namingPrefix, $knotName,
                $idempotencyStatement, ';', $N
                )"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:variable name="idempotencyStatement">
                    <xsl:if test="$attribute/@timeRange and $attribute/metadata/@idempotent = 'true'">
                        ...
                        $T, $T, 'd.', $anchorIdentity, ' = i.', $anchorIdentity, $N,
                        $T, 'AND', $N,
                        $T, $T, 'd.', $attributeName, ' = i.', $attributeName, $N,
                        $T, 'WHERE', $N,
                        $T, $T, 'd.', $anchorIdentity, ' is null'
                        )"/>
                    </xsl:if>
